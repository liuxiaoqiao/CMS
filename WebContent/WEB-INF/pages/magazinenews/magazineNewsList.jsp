...
<script type="text/javascript">
	...
	function jumpPage(jumpPage, pageSize) {
		...
		$.ajax({
			url : "magazineNews.do?action=queryMagazineNewsListByPageIndex",
			cache : false,
			type : "post",
			dataType : "text",
			data : {
				"pageIndex" : jumpPage,
				"pageSize" : pageSize,
				"magazineTitle" : $("#maginzeTitle").val(),
	...
	function createNewPageHTML(tableData) {
		...
		for ( var i = 0; i < tableData.length; i++) {
			...
			newPageHtml += nullToEmpty(tableData[i].magazineTitle);
			newPageHtml += "</td>";

			newPageHtml += "<td style='text-align:center;width:25%;'>";
			newPageHtml += nullToEmpty(tableData[i].magazineNum);
			newPageHtml += "</td>";

			newPageHtml += "<td style='text-align:center;width:10%;'>";
			newPageHtml += nullToEmpty(tableData[i].pressPerson);
			newPageHtml += "</td>";

			newPageHtml += "<td style='text-align:center;width:10%;'>";
			newPageHtml += nullToEmpty(tableData[i].magazineStatus);
			newPageHtml += "</td>";

			newPageHtml += "<td style='text-align:center;width:20%;'>";
			newPageHtml += "<a href='#' onclick=editMagazine('"+tableData[i].magazineID+"')>编辑</a>";
			newPageHtml += "<a href='#' style='margin-left:5px' onclick=deleteMagazine('"+tableData[i].magazineID+"')>删除</a>";
			newPageHtml += "<a href='#' style='margin-left:5px' onclick=editMagazine('"+tableData[i].magazineID+"')>审核</a>";
			newPageHtml += "</td>";
	...
	function editMagazine(magazineID) {
		document.location.href="${pageContext.request.contextPath}/magazineNews.do?action=turnToMagazineDetail&magazineID="+escape(magazineID);
	}
	function deleteMagazine(magazineID){
		...
		.confirm(
				...
				function(action) {
					if (action == "ok") {
						$.ajax({
							url : "magazineNews.do?action=deleteMagazine",
							cache : false,
							type : "GET",
							dataType : "json",
							data :{
	...
	function turnToAddMagazine(){
		document.location.href="${pageContext.request.contextPath}/magazineNews.do?action=turnToMagazineDetail&magazineID=";
	}
</script>
</head>
...
<div class="searchBox">
					...
					<ul>
						<li><label for="title">标题：</label><input type="text"
							id="maginzeTitle" class="text"> <label for="pressPerson">发稿人：</label><input
						...
						<li><label class="l">时间：</label> <input class="Wdate"
							...
							onclick="turnToAddMagazine()" class="btnBlue77_32">新增</a> <input
	...
	<%-- <fieldset>
		...
		<table id="searchTable">
			...
			<tr>
				...
				<input type="button" value="新增" onclick="turnToAddMagazine()" class="button">
				...
				<c:forEach items="${paginations.result}" var="p" varStatus="xh">
					<tr align="center">
						...
						<td><a href="#"  class="btn44_20 btnBlue" onclick="editMagazine('${p.magazineID}')">编辑 </a> <a href="#"
							 class="btn44_20 btnBlue" onclick="deleteMagazine('${p.magazineID}')">删除 </a> 
