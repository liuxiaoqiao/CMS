...
<script type="text/javascript">
	...
	function showAdd() {
		document.location.href = "${pageContext.request.contextPath}/specialColumn.do?action=turnToAddSpecialColumn&columnID=";
	}

	function showEdit(columnID) {
		document.location.href = "${pageContext.request.contextPath}/specialColumn.do?action=turnToAddSpecialColumn&columnID="
				+ columnID;
	}
	function deleteSpecialColumn(columnID) {
		asyncbox.confirm("确定删除该笔信息?", "信息窗口", function(action) {
			if (action == "ok") {
				$.ajax({
					url : "specialColumn.do?action=deleteSpecialColumn",
					cache : false,
					type : "GET",
					dataType : "text",
					data : {
	...
	function jumpPage(jumpPage, pageSize) {
		$.ajax({
			url : "specialColumn.do?action=querySpecialColumnListByPageIndex",
			cache : false,
			type : "POST",
			dataType : "text",
			data : {
				"pageIndex" : jumpPage,
				"pageSize" : pageSize,
				"columnTitle" : $("#columnTitle").val() == null ? ""
						: encodeURIComponent($("#columnTitle").val())
			},
			success : function(content) {
	...
	function createNewPageHTML(tableData) {
		...
		for ( var i = 0; i < tableData.length; i++) {
			...
			newPageHtml += "<td >";
			newPageHtml += tableData[i].COLUMNTITLE;
			newPageHtml += "</td>";

			newPageHtml += "<td >";
			newPageHtml += tableData[i].ISSHOW;
			newPageHtml += "</td>";

			newPageHtml += "<td style='word-break: break-all;text-align:left'>";
			newPageHtml += tableData[i].COLUMNURL;
			newPageHtml += "</td>";

			newPageHtml += "<td style='text-align:center'>";
			newPageHtml += "<a class='btn44_20 btnBlue' onclick=showEdit('"
					+ tableData[i].COLUMNID + "');return false;'>编辑</a>";
			newPageHtml += "<a class='btn44_20 btnBlue'  onclick=deleteSpecialColumn('"
					+ tableData[i].COLUMNID + "');>删除</a>";
			newPageHtml += "</td>";
...
<body style="background: white;">
	<form:form id="form">
		<div class="searchBox">
			<h3>专题专栏</h3>
			<ul>
				<li><label for="title">标题：</label><input class="text"
					type="text" id="columnTitle"> <a
		...
		<div>
			...
			<display:table class="infoList"
				...
				requestURI="${pageContext.request.contextPath}/specialColumn.do"
				partialList="enable">

				<display:column property="columnTitle" title="标题" />
				<display:column property="isShow" title="首页显示" />
				<display:column property="columnUrl"
					style="word-break: break-all;text-align:left" title="超接连地址" />

				<display:column title="操作">
					<div style="text-align: center">
						<a class='btn44_20 btnBlue'
							onclick="showEdit('${row.columnID}');return false;">编辑</a> <a
							class='btn44_20 btnBlue'
							onclick="deleteSpecialColumn('${row.columnID}');return false;">删除</a>
