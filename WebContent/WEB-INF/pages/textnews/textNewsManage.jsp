...
<script type="text/javascript">
	...
	function jumpPage(jumpPage, pageSize) {
		$.ajax({
			url : "textNews.do?action=queryTextNewsListByPageIndex",
			cache : false,
			type : "POST",
			dataType : "text",
			data : {
				"pageIndex" : jumpPage,
				"pageSize" : pageSize,
				"newsTitle" : $("#newsTitle").val() == null ? ""
						: encodeURIComponent($("#newsTitle").val()),
	...
	function createNewPageHTML(tableData) {
		...
		for ( var i = 0; i < tableData.length; i++) {
			...
			newPageHtml += "<td >";
			newPageHtml += "<a onclick=updateTextNewsView('"
					+ tableData[i].NEWSID + "')>编辑</a>";
			newPageHtml += "<a onclick=deleteTextNews('" + tableData[i].NEWSID
					+ "')>刪除</a>";
			newPageHtml += "<a onclick=approvalTextNews('"
					+ tableData[i].NEWSID + "')>审核</a>";
			newPageHtml += "<a onclick=distributTextNews('"
					+ tableData[i].NEWSID + "')>分发</a>";
			newPageHtml += "</td>";
	...
	function previewFormList(newsID) {
		var strURL = "${pageContext.request.contextPath}/textNews.do?action=previewFormList&newsID="
				+ newsID;
	...
	function approvalTextNews(newsID) {
		window.location.href = "${pageContext.request.contextPath}/textNews.do?action=approvalTextNews&newsID="
				+ newsID;
	}

	function distributTextNews(newsID) {
...
<body style="background: white;">
		...
		<div class="searchBox">
			...
			<ul>
				<li><label for="title">标题：</label><input type="text"
					id="newsTitle" class="text"> <label for="author">发稿人：</label><input
				...
				<li><label class="l">发稿时间：</label> <input class="Wdate"
					...
					onclick="insertTextNewsView()" class="btnBlue77_32">新增</a> <input
					...
					value="${programType}" /></li>
			</ul>
		</div>
		<%-- 
 	<fieldset id="homeSearch"><legend>交通新闻</legend> 
	标题:<input type="text" id="newsTitle">
    &nbsp;&nbsp;&nbsp;&nbsp;
	发稿人:<input type="text" id="entryUser">
	<br />
	发稿时间:
	<input class="Wdate"
					id="pressDateS"
					onfocus="WdatePicker({readOnly:true,dateFmt:'yyyy.MM.dd'})"
					size="12" maxlength="30" readonly="true" /> 
	&nbsp;至
		<input class="Wdate"
					id="pressDateE"
					onfocus="WdatePicker({readOnly:true,dateFmt:'yyyy.MM.dd'})"
	...
	<input type="button" value="新增" id="insertData" class="button" onclick="insertTextNewsView()" />
 </fieldset>
  ...
  --%>
		<div>
			...
			<display:table class="infoList"
				...
				<%--  <display:column property="newsTitle" title="标题"  style="text-align:center;"/>--%>
				<display:column title="标题">
					<a onclick="previewFormList('${row.newsID}')">${row.newsTitle}</a>
				</display:column>
				...
				<display:column title="操作">
					<div align="center">
						<a class="btn44_20 btnBlue"
							...
							onclick="deleteTextNews('${row.newsID}')">删除</a> <a
							class="btn44_20 btnBlue"
							onclick="approvalTextNews('${row.newsID}')">审核</a> <a
							class="btn44_20 btnBlue"
							onclick="distributTextNews('${row.newsID}')">分发</a>
					</div>
				</display:column>
